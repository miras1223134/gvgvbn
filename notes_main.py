...
notes_win = QWidget()
notes_win.setWindowTitle('заметки умные')
...
button_note_save = QPushButton('Сохранить замету')

field_tag = QLineEdit('')
field_tag.setPlaceholderText('Введите тег ...')
field_text = QTextEdit()
button_tag_add = QPushButton('Добавить к заметке')
button_tag_del = QPushButton('Окрепить от заметки')
...
def save_note():
    if list_notes.selectedItems():
        ...
        print('заметки для сохранения не выбрана!')
def del_note():
    if list_notes.selectedItems():
        key = list_notes.selectedItems()[0].text()
        del notes[key]
        ...
        print('Заметки для добавления тега не выбрана!')
...
def search_tag():
    ...
    if button_tag_search.text() == 'Искать заметку по тегу' and tag:
        ...
        button_tag_search.setText('СБОРНИК ПОИСК')
        list_notes.clear()
        list_tags.clear()
...
button_tag_add.clicked.connect(del_tag)
